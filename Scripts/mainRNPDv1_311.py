...
def Deps(force_reinstall):
    ...
    if not force_reinstall and os.path.exists('/usr/local/lib/python3.11/dist-packages/gradio'):
        ...
        depsinst("https://github.com/utmostmick0/sd_dependencies/blob/f2216938549c58f6d16363cdbe8650960839bc7e/rnpddeps-t2.tar.zst")
...
def downloadmodel_hf(Path_to_HuggingFace):
  ...
  if os.path.exists('unet/diffusion_pytorch_model.bin'):
    ...
    wget.download('https://raw.githubusercontent.com/utmostmick0/fast-stable-diffusion/main/Dreambooth/model_index.json')
    os.chdir('/notebooks')
    clear_output()
    done()
